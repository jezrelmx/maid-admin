...
//Rare exceptions to indicate a bug in the _maid_admin code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_maid_admin = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_maid_admin]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.maid_admin.module"
